<h1 align="center">你好 👋，我是王文博</h1>
<h3 align="center">🤖 热爱AI Agent的产品经理 | LLM爱好者 | 产品驱动的创新者</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=wangwenbo0305&label=Profile%20views&color=0e75b6&style=flat" alt="wangwenbo0305" />
</p>

---

## 🧑‍💻 关于我

- 🎯 **AI Agent产品经理**，在LLM应用、RAG系统和多智能体架构方面有丰富的实战经验
- 🏢 曾在**百度**（品牌广告）和**第四范式**（AI平台）担任产品经理
- 🚀 热衷于构建连接技术与商业价值的**智能Agent**
- 📊 具备数据驱动产品决策的坚实背景
- 🌱 目前专注于研究先进的多智能体编排和自主AI系统

---

## 🏆 核心成就

| 项目 | 成果 |
|------|------|
| **百度品牌广告AI Agent** | 物料迁移时间从小时级压缩至分钟级，迁移成功率92% |
| **多模态风控审核系统** | 审核效率提升87.5%，误判率下降70.8% |
| **第四范式私有化MaaS平台** | 问答准确率98.4%，Top-3召回率97.5% |
| **航空多智能体系统** | 手册检索效率提升70%，平均排故时间缩短40% |

---

## 🛠️ 技术栈

### AI与大模型
<p align="left">
  <img src="https://img.shields.io/badge/LLM-OpenAI%2FClaude%2FDeepSeek-blue?style=for-the-badge&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/RAG-LangChain%2FLlamaIndex-green?style=for-the-badge&logo=langchain&logoColor=white" />
  <img src="https://img.shields.io/badge/Agent-OpenClaw%2FHermes-orange?style=for-the-badge&logo=robot&logoColor=white" />
  <img src="https://img.shields.io/badge/微调-LoRA%2FSFT%2FRLHF-red?style=for-the-badge&logo=pytorch&logoColor=white" />
</p>


### 能力与工具
<p align="left">
  <img src="https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-007ACC?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-007ACC?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Axure-000000?style=for-the-badge&logo=axure&logoColor=white" />
  <img src="https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=jira&logoColor=white" />
  <img src="https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white" />
</p>


---

## 🚀 代表项目

### TalkingData广告点击欺诈检测竞赛（Kaggle）
- 处理2亿条点击数据，解决正负样本比例1:33的极度不平衡分类问题
- 采用LightGBM算法构建模型，结合高维统计特征工程
- 达到**0.96 AUC**，欺诈点击识别**召回率93%**

### 未完待续


---

## 🏅 资质认证

- **阿里云大数据助理工程师（ACA）**

---

## 📫 联系我

<p align="center">
  <a href="mailto:wangwenbo030509@163.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-wangwenbo030509@163.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

---

## 🎯 2026年目标

- [ ] 深入研究自主多智能体系统和Agentic Workflow
- [ ] 为开源LLM/Agent框架贡献代码
- [ ] 构建个人AI Agent作品集，展示端到端产品思维
- [ ] 掌握先进的RLHF和模型对齐技术

---


<p align="center">
  <em>"打造放大人类潜力的智能Agent。" 🚀</em>
</p>
